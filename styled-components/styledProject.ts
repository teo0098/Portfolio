...
export const StyledProject: StyledComponent<
  'section',
  any,
  OrderProps
> = styled.section<OrderProps>`
  ...
  ${({ theme }: { theme: ThemeInterface }) => theme.media.desktop} {
    ...
    flex-direction: ${({ index }) =>
      index != undefined && index % 2 === 0 ? 'row' : 'row-reverse'};
    justify-content: space-between;
  }
`;

export const StyledImg: StyledComponent<
  'img',
  any,
  OrderProps
> = styled.img<OrderProps>`
  max-width: 100%;
  height: auto;
  object-fit: cover;
  border-radius: 5px;
  box-shadow: ${({ theme }: { theme: ThemeInterface }) =>
    `20px 20px 12px -12px ${theme.colors.dark}`};
...
export const StyledButton: StyledComponent<'button', any> = styled.button`
  ...
  background-color: ${({ theme }: { theme: ThemeInterface }) =>
    theme.colors.light};
  box-shadow: ${({ theme }: { theme: ThemeInterface }) =>
    `0 8px 6px -6px ${theme.colors.dark}`};
  font-size: 17px;
  ...
  ${({ theme }: { theme: ThemeInterface }) => theme.media.tablet} {
    ...
    :hover {
      ...
      box-shadow: ${({ theme }: { theme: ThemeInterface }) =>
        `0 16px 12px -12px ${theme.colors.dark}`};
    }
  }
`;
